...
function makeTwo() {
	...
	try {
		const e = 1;
		console.log('Изначально:', e);
		e = 5;
		console.log('После измененения', e)
	}
	catch(error){
...
function makeNineteen() {
	...
	const nineteenFinally = document.getElementById('result19a');
	...
	try{
		...
		nineteenFinally.textContent = "finally19";
	}
}
...
function makeTwenty() {
	try{
		...
		document.getElementById('result20').textContent = error.message;
	}
	finally{
...
function makeTwentyOne() {
	try{
		...
		document.getElementById('result21').textContent = error.message;
	}
	finally{
...
function makeTwentyFour() {
	...
	try{
		let divisor = 0;
		if (divisor === 0) {
			throw new Error('Невозможно поделить на ноль')
		}
...
function makeTwentySix() {
	...
	setTimeout(() => {
		try{
			...
			resultTwentySix.textContent  = 'Ошибка при обработке асинхронного кода';
		}
	}, 1000);
}
...
function makeTwentyEight() {
    ...
    try {
        ...
        resultTwentyEight.textContent = 'Ошибка на верхнем уровне: ' + error;
